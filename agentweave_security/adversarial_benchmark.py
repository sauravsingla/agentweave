...
def run_adversarial_scenario(
    ...
    Token and latency values are modeled comparison proxies only. This harness
    does not alter frozen AgentWeave routing evidence or claim production security.
    """
    ...
    if condition is ExposureCondition.ALL_TOOLS:
        ...
        observe_narrowing_stage(
            ...
            reason_codes={
                name: (
                    "unauthorized_prefilter"
                    if not tool_map[name].authorized
    ...
    if selected is not None:
        ...
        decision = gate.authorize(action=selected.name, context={"scenario": scenario.name})
        ...
        # The fail-closed gate guarantees denied actions never reach execution.
        unauthorized_executions = 0
